//-----------------------------------------------------------------------
// Version:        2.1
// Template name:  pluto wallet - Wallet & Banking HTML Mobile Template
// Item URL :      https://themeforest.net/item/pluto wallet-wallet-banking-html-mobile-template/25738217
// Author:         Bragher
...
goBackButton.forEach(function (el) {
    el.addEventListener("click", function () {
        if (pluto wallet.Animation.goBack) {
            goBackAnimation();
        }
        else {
            window.history.go(-1);
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("pluto walletAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("pluto walletAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("pluto walletDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (pluto wallet.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (pluto wallet.Dark_Mode.local_mode.enable) {
    var nightStart = pluto wallet.Dark_Mode.local_mode.start_time;
    var nightEnd = pluto wallet.Dark_Mode.local_mode.end_time;
...
if (pluto wallet.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("pluto walletDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("pluto walletDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("pluto walletDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%cpluto wallet", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(pluto wallet.PWA.enable, "PWA")
    testModeMsg(pluto wallet.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(pluto wallet.Dark_Mode.local_mode.enable, "Local dark mode (between " + pluto wallet.Dark_Mode.local_mode.start_time + ":00 and " + pluto wallet.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(pluto wallet.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(pluto wallet.RTL.enable, "RTL")
    testModeMsg(pluto wallet.Test.enable, "Test mode")
    testModeMsg(pluto wallet.Test.alert, "Test mode alert")
    ...
    testModeMsg(pluto wallet.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = pluto wallet.Test.word;
    var value = "";
        ...
        if (value == word || value === word) {
            ...
            if (pluto wallet.Test.alert) {
                ...
                var alert =
                    ...
                    +
                    pluto wallet.Test.alertMessage
                    +
                    "</strong></div></div></div>"
